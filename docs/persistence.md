...
Report partial persistence honestly: Git and Drive cannot be committed atomically together. Read back uncertain operations through the native binding where one exists, else through the provider tools that performed the transfer, and use provider idempotency/reconciliation to avoid duplicate uploads. No blind retries or default remote deletions. QMD indexes and model downloads do not belong in publication.
