...
Settings are agent-readable policy. Applying them is agent-owned work through existing provider tools. There is no routing/upload pipeline or scheduler in this plugin. The default is local storage with backup off. Setting a mode does not move existing files or authorize publication outside the selected library.
